...
Definition eval_relu (b_var f_var aux_var : 'I_n) (x : 'rV[R]_n) : bool :=
  [&& b_var != f_var,
      b_var != aux_var,
      f_var != aux_var,
      (x 0 f_var == compute_relu (x 0 b_var)) &
      (x 0 aux_var + x 0 b_var - x 0 f_var == 0)
  ].
...
(*[@@fc]*)
Lemma eval_relu_different_var (b_var f_var a_var : 'I_n) (x : 'rV[R]_n) :
  eval_relu b_var f_var a_var x ->
  [&& f_var != b_var, f_var != a_var & b_var != a_var].
Proof.
  rewrite /eval_relu.
  move=> /andP [] H1 /andP [] H2 /andP [] H3 _.
  rewrite /andb.
  case E: (f_var != b_var).
  - by destruct (f_var != a_var).
  - rewrite <- eq_sym in H1. by rewrite E in H1.
Qed.
