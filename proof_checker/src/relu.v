...
Definition relu (x : R) : R := Num.max x 0.
...
Definition eval_relu (b_var f_var aux_var : 'I_n.+1) (x : 'rV[R]_n.+1) : bool :=
  [&& b_var != f_var, 
      b_var != aux_var, 
      f_var != aux_var &
      (x 0 b_var == relu (x 0 f_var)) && (x 0 aux_var + x 0 b_var - x 0 f_var == 0)
  ].
...
Lemma eval_relu_different_var (xs : 'rV[R]_n.+1) (f_var b_var a_var : 'I_n.+1) :
  eval_relu f_var b_var a_var xs ->
  [&& b_var != f_var, b_var != a_var & f_var != a_var].
  rewrite /eval_relu.
